...
export function returnPatients(patients: any[]) {
  const responseObj = {
    items: patients,
...
export function returnOrders(orders: any[]) {
  const responseObj = {
    items: orders,
    total: orders.length
  };
...
export function returnUsers(users: any){
  const responseObj = {
    items: users.data,
    total: users.total
  };
...
export function searchInArray(incomingArray: any[], searchTerm: string = ''): any[] {
  ...
  const result = incomingArray.filter(row => {
    ...
    for (const keyName of keys) {
        if (row[keyName].toString().indexOf(searchTerm) > -1) {
