...
export class ChangePasswordComponent implements OnInit, OnDestroy {
  ...
  loadForm() {
    this.formGroup = this.fb.group({
      currentPassword: [this.user.password, [Validators.required, Validators.minLength(6)]],
  ...
  save() {
    ...
    let login = {email: this.user.email, password: this.formGroup.value.currentPassword ,newPassword: this.user.password};
    
    this.httpService.changePassword(login)
  ...
  isControlInvalid(controlName: string): boolean {
    ...
    return control.invalid && (control.dirty || control.touched) || checkLength;
  }
