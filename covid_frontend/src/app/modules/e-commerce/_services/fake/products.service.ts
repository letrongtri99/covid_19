...
import { returnOrders } from '../../../../_fake/fake-helpers/http-extenstions';
...
export class ProductsService extends TableService<Product> implements OnDestroy {
  ...
  find(tableState: ITableState): Observable<TableResponseModel<Product>> {
    let url = `${environment.host}/orders`
    return this.http.post<Product[]>(url, tableState).pipe(
      map((response: Product[]) => {
        const filteredResult = returnOrders(response);
